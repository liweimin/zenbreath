...
| TC-003 | P0  | 是   | 会话中切换 5 种自然环境音                         | 不报错；音频上下文保持 `running`      |
...
| TC-020 | P0  | 是   | 提示音模式切换（真实采样/合成）                     | 模式切换后播放正常且无报错               |
| TC-021 | P0  | 是   | 默认提示音模式为真实采样                            | 首次加载时模式状态正确                   |
| TC-022 | P0  | 是   | 三段真实采样提示音在运行中可触发                      | `inhale/hold/exhale` 均有可听触发         |
| TC-023 | P0  | 是   | 真实采样与合成模式切换后会话不中断                     | 会话状态保持运行，无异常日志               |

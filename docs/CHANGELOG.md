...
- 新增提示音模式切换（`真实采样 / 合成`），默认使用真实采样。
- 新增真实采样提示音素材（吸气磬 / 屏气水滴 / 吐气鼓）。
- 新增自动化用例 `TC-020` ~ `TC-023`，覆盖提示音模式默认值、模式切换与会话不中断。

### Changed
- 环境音切换升级为平滑 crossfade，默认音量从 `40` 下调为 `30`，降低初始听感刺激。
- 移动端设置面板与底部按钮布局优化（`390x844` 关键路径可用）。
- 五种环境音替换为自然声场素材（溪流 / 细雨 / 雨幕 / 山风 / 雷雨）。
- 音频引擎支持“采样优先、合成兜底”，并兼容 `file://` 本地测试场景。

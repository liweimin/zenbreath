...
- 音频迭代探索目标：验证“真实采样提示音（磬/水滴/鼓）+ 自然背景音（雨声/水声）”方案的可行性与听感上限。

## 5. 范围定义
### 5.1 In Scope
- 呼吸模式选择：预设 + 自定义四段时长。
- 会话时长：5 分、10 分、自定义、无尽模式。
- 环境音：5 种自然声场（溪流/细雨/雨幕/山风/雷雨）+ 音量开关。
...
- FR-13 提示音模式切换  
  系统应提供两套提示音模式：`真实采样` 与 `合成`，默认使用 `真实采样`，用户可手动切换。

- FR-14 真实采样提示音基线  
  真实采样模式下，吸气/屏气/吐气应分别使用约定素材（磬/水滴/中式鼓单击），并保证起音立即可感知。

- FR-15 自然环境音替换  
  系统应将现有五种环境音替换为自然系背景音（共 5 种），并保留环境音切换与音量控制能力。
...
- AC-10 设置面板可切换提示音模式，默认应为“真实采样”。
- AC-11 在真实采样模式下，吸气/屏气/吐气均可正常播放且起音立即可听见。
- AC-12 切换到合成模式后，提示音应回退为原合成逻辑且无报错。
- AC-13 五种环境音替换为自然声后，切换 30 次仍不应出现静音锁死或运行错误。
